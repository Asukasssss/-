...
最新数值批次已完成：33 名经 GEO individual 核对的 I–IV 期患者，预设 674 条关系中 652 条可计算，新的关联 q<0.05 为 0 条（最小 q=0.326）。446 个基因可做33对肿瘤—正常RNA比较，314 个 q<0.05，其中146上升、168下降。RNA差异与肿瘤内部相关不是同一问题，不据此认定全部候选无效。

全部37个作者Tumor（含3腺瘤、1个0期）敏感性及可用性敏感性均为0条 q<0.05。可用性当前版本为 v2：437条相同输入复用主分析，215条样本减少项保留原统计；所有974候选行保留。652条均完成逐一剔除患者检查。协变量调整、功能、外部验证尚未完成。
...
- 功能干预/依赖和外部独立资料尚未完成；反应注释不是COAD功能证明。下一轮按完整候选取证，不把关联显著作为唯一入场门槛。
