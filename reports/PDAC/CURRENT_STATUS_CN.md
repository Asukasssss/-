# PDAC 当前进度：髓系 inferCNV 补充结果（2026-09-26）

按用户新增请求已完成三套队列31个作者单位、14,133个肿瘤髓系细胞、93次inferCNV参考分析；120个一致CNV异常候选不等于已确认恶性细胞。三队列符合组间比较的单位数为0、1、0，均不足预设5个，SLC6A6比较全部NOT_EVALUABLE，未计算P/q。独立数值核查和4项统计函数测试通过。

[本次完整结果、对照和图表](../../results/PDAC/06_EXTERNAL/20260925T141744Z_myeloid_infercnv_v1/README_CN.md)。数值批次完成不等于外部验证或髓系恶性身份完成，整体研究缺项继续保留。PR #2仍为草稿、未合并。

以下为既有统一报告v5的归档范围；其中“不追加CNV”描述当时批次，本次按用户新请求单独扩展，不改写历史统计。
