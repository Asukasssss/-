...
def main():
 ...
 members=pd.read_csv(S/'kegg_members.tsv',sep='\t')
 # One KEGG GENE entry supplies only KO/EC, with no gene symbol. Keep its ID,
 # not the KO annotation text, as an explicitly unresolved placeholder.
 unresolved=members.gene.str.startswith('[',na=False)
 members.loc[unresolved,'gene']='UNRESOLVED_KEGG_'+members.loc[unresolved,'kegg_gene_id'].str.replace('hsa:','',regex=False)
 assert len(members)==spec['planned_members'] and members.gene.is_unique
