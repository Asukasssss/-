"""Generate Chinese delivery report from fixed aggregate outputs."""
from pathlib import Path
import sys,json
import pandas as pd
P=Path(sys.argv[1]);s=pd.read_csv(P/'analysis_summary.tsv',sep='\t');g=pd.read_csv(P/'group_summary.tsv',sep='\t');qc=pd.read_csv(P/'depth_confounding.tsv',sep='\t')
def table(d):
 return '|'+ '|'.join(d.columns)+'|\n|'+'|'.join(['---']*len(d.columns))+'|\n'+'\n'.join('|'+ '|'.join(str(x) for x in row)+'|' for row in d.itertuples(index=False,name=None))
report='''# BRCA：LYPLA1高低表达癌上皮的差异基因与富集
...
### 分组
'''+table(g[['cohort','group','n_cells','positive_median_cut','n_donors','median_counts','median_detected_genes']])+'''
...
'''
(P/'README_CN.md').write_text(report,encoding='utf-8')
