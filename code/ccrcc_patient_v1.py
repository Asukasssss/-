...
R=Path('/public3/xuzx/Cancer/pancancer_metabolomics_direct_matrix_20260716');S=R/'data/candidates/camp_primary_tissue_multicancer';V='ccrcc_patient_v1'
...
def corr(x,y,key,strata=None):
 ...
 xx=stats.rankdata(x[boot],axis=1);yy=stats.rankdata(y[boot],axis=1);xx-=xx.mean(1,keepdims=True);yy-=yy.mean(1,keepdims=True)
...
def main(out,commit):
 ...
 spec=dict(version=V,code_commit=commit,unit='explicit author SUBJECT_ID, regions averaged separately within tissue',
  ...
  BH='all evaluable tests independently per family, including all125 current relations/current94genes;history-only gene family separate',
