...
def main(out,commit):
 ...
 summary=dict(status='DONE',ccRCC4_mapped_specimens=76,excluded_medullary_specimens=len(excluded),
  ...
  ccRCC3_status='NEEDS_REVIEW',ccRCC3_reason='author_patient_link_absent_from_master_mapping_and_original_ccRCC3_workbook_absent_from_archive;do_not_infer_from_ID',
 ...
 spec=dict(version=V,code_commit=commit,cancer='ccRCC',cohort='ccRCC4',
  inclusion='MasterMapping Dataset=ccRCC4, Histology=Clear Cell, exact original SAMPLE_NAME join; source cohorts kept separate',
  ...
  limitations=['small n','treated cohort','within-patient region mean estimand','same-cohort selection','ccRCC3 patient identity unresolved','genotype identity not checked'],
 ...
 (pub/'README_CN.md').write_text(f'''# ccRCC样本设计与历史结果\n\n问题：能否按BRCA主线建立患者独立的ccRCC分析？\n\n输入：CAMP作者矩阵、MasterMapping和ccRCC4原始工作簿；逐样本表只留server165。\n\n实际结果：ccRCC4原76标本包含2个髓质癌标本，排除后{len(m)}个透明细胞癌标本、{m.SUBJECT_ID.nunique()}位作者患者、{len(paired)}对患者。多个区域先在患者×组织内等权取均值。全矩阵{len(mat)}特征，作者两组织均保留{len(retained)}项。原癌种冻结{len(frozen)}项及其P/q原样保存。\n\n新手解释：标本数不是患者数；同一患者的多个区域不能当作多位患者增加样本量。\n\n限制：治疗背景混合，主要为免疫治疗；不是治疗效果比较。ccRCC3缺少明确患者连接，不能按样本名猜配对，暂不计算患者检验。矩阵内未被作者跨组学表确认的标本不擅自扩入。\n\n当前决定：按已冻结设计执行ccRCC4患者均值配对分析；保留ccRCC3状态。\n\n下一步：全量发现、直接关系、患者关联、RNA背景、全候选单细胞来源。\n\n复现：python ccrcc_discovery_v1.py --out 新服务器运行目录 --code-commit 提交号；需预先放入source/ccRCC4_original.xlsx。\n''',encoding='utf-8')
