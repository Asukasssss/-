...
def plot_results(r,points,out):
 ...
 for i,co in enumerate(['Wu2021','Pal2021_reprocessed']):
  for j,gene in enumerate(['LYPLA1','LYPLA2']):
   ...
   ax.set_ylabel('Fixed PC-remodelling transcript score\n(mean within-study gene z-scores)')
 ...
 for j,mode in enumerate(['processed','author_available']):
  ...
  ax.set_xlabel('Bulk tissue PC-remodelling transcript score');ax.set_ylabel('Measured LPC 16:0 (unchanged author scale)')
 ...
 with PdfPages(out/'LYPLA_PCscore_results.pdf') as pdf:
...
def main():
 ...
 def test(co,typ,x,y,labels,gene='NA',cov=None):
  ...
  row.update(cancer='BRCA',cohort=co,stage_id='06_EXTERNAL' if co!='CAMP' else ('04_ROBUSTNESS' if cov is not None else '03_PATIENT'),run_id=R.name,
             ...
             effect_type='partial_rank_correlation' if cov is not None else 'Spearman_rho',test_family='PCscore_BH8',family_n_evaluable=8,
             status='NOT_EVALUABLE',reason='less_than8_or_constant',source_id='frozen_Reactome_R-HSA-1482788',test_id=key,score_gene_n=len(genes))
 ...
 pathway=set(pd.read_csv(S/'reactome_members.tsv',sep='\t').gene)
 ...
 with pd.ExcelWriter(P/'LYPLA_PCscore_results.xlsx',engine='openpyxl') as w:
