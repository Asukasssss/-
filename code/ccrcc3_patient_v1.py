...
R=Path('/public3/xuzx/Cancer/pancancer_metabolomics_direct_matrix_20260716');S=R/'data/candidates/camp_primary_tissue_multicancer';V='ccrcc3_patient_v1'
...
def row(out,fam,gene,key='NA',name='NA'):
 d=dict.fromkeys(PREFIX,np.nan);d.update(cancer='ccRCC',cohort='ccRCC3',stage_id='03_PATIENT',run_id=out.name,analysis_version=V,analysis_type=fam,metabolite_key=key,metabolite_name=name,gene=gene,unit='author_patient',test_family=fam,status='NOT_EVALUABLE',reason='gene_not_measured',source_id='CAMP_ccRCC3_SUBJECT_ID');return d
...
def main(out,commit):
 ...
 metp=S/'processed_metabolomics/PreprocessedData_ccRCC3.xlsx';mat=pd.read_excel(metp,sheet_name='data_imputed',index_col=0);raw=pd.read_excel(metp,sheet_name='data',index_col=0)
