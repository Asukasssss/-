...
d['selected']=(d.p_value<.05)&(d.log2FC.abs()>=.5)
d['depth_selected']=(d.depth_p_value<.05)&(d.depth_log2FC.abs()>=.5)
common=dict(cancer='COAD',cohort='Uhlitz_GSE166555',stage_id='06_EXTERNAL',run_id=out.name,analysis_version='COAD_LYPLA1_highlow_nominal_v2',
...
spec.update(analysis_version='COAD_LYPLA1_highlow_nominal_v2',user_change='User explicitly requested no FDR; nominal P only',
    DE_selection='P < 0.05 and absolute log2FC >= 0.5',gene_family='All 11058 tested genes retained; no new BH; prior Q values archived only',
...
summary=dict(status='PASS',gene_P_below_05=int((d.p_value<.05).sum()),selected_high=int((d.selected&(d.log2FC>0)).sum()),selected_low=int((d.selected&(d.log2FC<0)).sum()),
