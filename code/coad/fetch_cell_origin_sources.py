...
def main():
 p=argparse.ArgumentParser();p.add_argument('--data-dir',type=Path,required=True);p.add_argument('--mode',choices=['metadata','matrices','author'],required=True);a=p.parse_args();d=a.data_dir.resolve();assert d.parent==ROOT/'data/candidates';d.mkdir(exist_ok=True)
 ...
 if a.mode=='author':
  base='https://portals.broadinstitute.org/crc-immune-hubs/extra/data/colon10x_default/'
  urls={'pelka_author_metadata.csv.gz':base+'colon10x_default_dZ_metatable.csv.gz','pelka_author_annot.csv.gz':base+'colon10x_default_dZ_annot.csv.gz'}
  for stem in ['dDvec_batchID.csv.gz','dDvec_sampleID.csv.gz','dDvec_geneID.csv.gz','dDvec_ensgID.csv.gz','dSp_rawCount.h5']:
   name='colon10x_default_'+stem;urls[name]=base+name
 def fetch(item):
