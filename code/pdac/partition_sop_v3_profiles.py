...
def main():
    ...
    r=SOURCE/'README_CN.md';r.write_text(r.read_text()+f'\n\n完整来源表为{sum(x["rows"] for x in rows):,}行，按队列无损分为{len(rows)}个TSV以遵守单文件5 MB限制。字段、精度、顺序保持；分片清单及原逻辑表哈希见[sc_celltype_profile_parts.tsv](sc_celltype_profile_parts.tsv)。规范中的sc_celltype_profiles.tsv指这些分片合并的逻辑表。\n',encoding='utf8',newline='\n')
