...
538个基因三队列均有状态行。三队列全类别唯一第一来源相同：{s['three_cohort_same_top']}；同时满足各队列bootstrap≥0.70及检出≥0.05：{s['stable_source_genes']}；共同合格类别中第一来源一致：{s['shared_category_same_top']}。不能把描述性一致称为疾病差异显著复现。
...
## 新手解释
A为内部条件满足且稳定上皮/导管来源，B为同样内部条件但非上皮来源；C保留生化候选及提示/弱关联或来源未稳定者，不能统称阴性；D标明条件映射、缺测、敏感性、低检出等具体缺口。RNA差异不显著不会被删除。A/B仍是探索性，不等于治疗靶点、恶性细胞特异或机制验证。
