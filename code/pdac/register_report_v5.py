"""Register completed scoped PDAC report batches without changing historical stage rows."""
from pathlib import Path
import pandas as pd
R=Path(__file__).resolve().parents[2]
path=R/'coordination/stages/PDAC.tsv';d=pd.read_csv(path,sep='\t',dtype=str,keep_default_na=False)
rows=[]
for run,version,scope,code,reason in [
 ('20260922T135400Z_report_adapter_v5','PDAC_report_adapter_v5','307metabolites;715relations;687genes;fixed presentation inputs','code/pdac/prepare_report_v5.py','Immutable adapter2971100 verified remotely; no numerical recomputation'),
 ('20260922T140000Z_unified_report_v5','PDAC_unified_report_v5','16page report;16sheet workbook;81page source heatmaps;all687 genes','code/pdac/render_report_v5.py','Presentation complete with explicit malignant identity/subtype/UMAP/clinical independence gaps')]:
 rows.append(['PDAC','07_INTEGRATION',run,version,'DONE',scope,'results/PDAC/07_INTEGRATION/'+run,code,'analysis/pdac-initial',reason,'Read complete relation-specific evidence; no automatic mechanism extension'])
new=pd.DataFrame(rows,columns=d.columns)
d=pd.concat([d[~d.run_id.isin(new.run_id)],new],ignore_index=True).sort_values(['stage_id','run_id'])
d.to_csv(path,sep='\t',index=False,lineterminator='\n')
report=R/'reports/PDAC/CURRENT_STATUS_CN.md'
report.write_text('''# PDAC 当前进度：统一报告 v5
...
print('PDAC_REPORT_STAGE_REGISTERED')
