...
def clean(df):return json.loads(df.to_json(orient='records',force_ascii=False))
def main():
 ...
 c=pd.read_csv(base/'candidate_scRNA_appended.tsv',sep='\t');c=c.sort_values(['candidate_class','mapping_tier','CAMP_q','CAMP_p','metabolite_name','gene'],na_position='last')
