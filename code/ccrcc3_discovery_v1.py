"""ccRCC3 patient-level discovery; ccRCC3 remains gated by author patient mapping."""
...
def main(out,commit):
 ...
 source=out/'source/MetaData_M4.tsv'
 mp=S/'metadata/MasterMapping_MetImmune_03_16_2022_release.csv'
 a=pd.read_csv(source,sep='\t',dtype=str)
 a['SAMPLE_NAME_AUTHOR']=a.SAMPLE_NAME
 a['SAMPLE_NAME']=a.SAMPLE_NAME.str.replace('.','-',regex=False)
 a['SAMPLE_DESCRIPTION']=a.TISSUE_STATUS
 assert a.SAMPLE_NAME.is_unique and a.SUBJECT_ID.notna().all()
 mapping=pd.read_csv(mp,dtype=str)
 m=mapping[mapping.Dataset.eq('ccRCC3')].merge(a,left_on='MetabID',right_on='SAMPLE_NAME',validate='one_to_one')
 assert len(m)==114 and m.MetabID.is_unique and m.RNAID.is_unique
 ...
 excluded=m[~m.Histology.eq('Clear Cell')].copy();m=m[m.Histology.eq('Clear Cell')].copy()
 ...
 path=S/'processed_metabolomics/PreprocessedData_ccRCC3.xlsx';x=pd.ExcelFile(path)
 ...
 for tissue,d in [('Tumor',ft),('Normal',fn)]:
  ...
  assert set(d.columns)==set(mapping.loc[mapping.Dataset.eq('ccRCC3')&mapping.TN.eq(tissue),'MetabID'])
 anno=pd.read_excel(x,sheet_name='metanno',index_col=0)
 anno.index=anno.index.astype(str).str.strip()
 oldpath=R/'results/tables/cohort_effects.tsv';old=pd.read_csv(oldpath,sep='\t');old=old[old.dataset.isin(['ccRCC3','ccRCC3'])]
 ...
 old4=old[old.dataset.eq('ccRCC3')];assert old4.feature_name.is_unique
 inv=pd.DataFrame({'metabolite_name':mat.index})
 inv['feature_id']=inv.metabolite_name.map(lambda z:'ccRCC3_FEATURE:'+__import__('hashlib').sha256(z.encode()).hexdigest()[:16])
 ...
 summary=dict(status='DONE',ccRCC3_mapped_specimens=114,excluded_non_clear_cell_specimens=len(excluded),
  ccRCC3_clear_cell_specimens=len(m),tumor_specimens=int(m.TN.eq('Tumor').sum()),normal_specimens=int(m.TN.eq('Normal').sum()),
  ...
  original_matrix_specimens=len(mat.columns),unmapped_histology_or_rna_link_not_extended=len(mat.columns)-114,
  ...
  ccRCC3_status='DONE',ccRCC3_reason='MetaData_M4 author SUBJECT_ID joined one_to_one after dot_to_hyphen formatting only;all tissue labels concordant',
 ...
 spec=dict(version=V,code_commit=commit,cancer='ccRCC',cohort='ccRCC3',
  inclusion='MasterMapping Dataset=ccRCC3, Histology=Clear Cell, exact original SAMPLE_NAME join; source cohorts kept separate',
  patient_id='Zenodo8066511 All Data.RData MetaData_M4 SUBJECT_ID, explicitly supplied by authors;SAMPLE_NAME dot-to-hyphen one-to-one normalization',
  ...
  limitations=['small n','mixed antiPD1 and untreated cohort','within-patient region mean estimand','same-cohort selection','cross-cohort patient independence not established','genotype identity not checked'],
 ...
 save(pd.DataFrame([dict(audit_item=k,value=v,status='DONE') for k,v in summary.items()]),pub/'sample_audit_summary.tsv')
 ...
 sources=[source,mp,path,oldpath,cancerpath,Path(__file__),Path(__file__).with_name('ccrcc_stats_v1.py')]
 ...
 for i,r in inv[inv.retained_both_author_filters].iterrows():
  ...
  for fam,mask in [('METAB_PAIRED_PRIMARY',finite),('METAB_PAIRED_AVAILABLE',finite&avail)]:
   d=dict.fromkeys(PREFIX,np.nan);d.update(cancer='ccRCC',cohort='ccRCC3',stage_id='04_ROBUSTNESS',run_id=out.name,analysis_version=V,analysis_type='patient_region_mean_paired',metabolite_key=r.metabolite_key,metabolite_name=name,gene='NA',unit='author_patient',test_family=fam,source_id='CAMP_ccRCC3_original_SUBJECT_ID',feature_id=r.feature_id,n_pairs_total=len(paired),n_both_available=int(avail.sum()),mask_semantics=spec['available_sensitivity'])
