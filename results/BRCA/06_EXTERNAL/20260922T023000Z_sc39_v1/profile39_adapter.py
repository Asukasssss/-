...
def run(root, config):
    ...
    assert len(genes) == len(set(genes)) == 39
    ...
    with h5py.File(source, 'r') as h:
        ...
        coverage = []
        for gene in genes:
            positions = np.flatnonzero(var.index.astype(str).str.split('.').str[0].isin(['ENSG00000176454', 'ENSG00000291994'])) if gene == 'LPCAT4' else np.flatnonzero(names.values == gene)
            ok = len(positions) == 1
        ...
        for key, labels in cfg.get('exclude_labels', {}).items():
            selected &= ~obs[key].astype(str).isin(labels).values
        ...
        logsum = np.zeros((ng, 39), dtype=np.float64)
        detected = np.zeros((ng, 39), dtype=np.int64)
        rawsum = np.zeros((ng, 39), dtype=np.float64)
        ...
        for start in range(0, len(obs), cfg['chunk_cells']):
            ...
            assert (lib > 0).all()
            y = np.zeros((len(lib), 39), dtype=np.float64)
        ...
        for partition, block in partitions:
            block = block.copy()
            block['wmean'] = block.mean_log1p10k * block.n_cells
            block['wdetect'] = block.detection_fraction * block.n_cells
            pooled = block.groupby(['donor', 'celltype', 'gene'], as_index=False)[
                ['n_cells', 'wmean', 'wdetect']].sum(min_count=1)
            pooled['mean'] = pooled.wmean / pooled.n_cells
            pooled['detect'] = pooled.wdetect / pooled.n_cells
            for (celltype, gene), v in pooled.groupby(['celltype', 'gene'], sort=True):
                ...
                summary_rows.append({'cancer':'BRCA', 'cohort':cohort, 'stage_id':'06_EXTERNAL',
                    'run_id':root.name, 'analysis_version':'sc39_v1', 'analysis_type':'descriptive_cell_source',
        ...
        audit = {'cohort':cohort,'status':'DONE','source_shape':list(map(int,shape)),
            ...
            'gene_count':39,'genes_unique_in_dictionary':int(valid_genes.sum()),
            ...
            'public_rows':len(result),'all39_retained':set(result.gene)==set(genes),
