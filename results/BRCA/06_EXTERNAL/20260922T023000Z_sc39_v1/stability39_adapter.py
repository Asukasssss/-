"""Source-label resampling of ALL39 cell-source summaries; no new tests."""
import os
...
def one(d,cohort,partition,seed):
    ...
    means=mean.reshape(len(types),39);detect=md.reshape(len(types),39)
    boots=boot.reshape(1000,len(types),39);loos=loo.reshape(len(donors),len(types),39)
    rows=[];dist=[]
    for j,gene in enumerate(genes):
        ...
        for k,t in enumerate(types):
            ix=k*39+j;v=x[:,ix];v=v[np.isfinite(v)];b=boot[:,ix];b=b[np.isfinite(b)]
            good=len(v)>=3
...
def main(root,previous):
    ...
    save(r,out/'all39_source_stability.tsv');save(z,out/'all39_source_distributions.tsv')
    ...
    ax.set_yticks(range(39));ax.set_yticklabels(genes,fontsize=7);ax.set_xticks(range(3));ax.set_xticklabels(cohorts,rotation=25,ha='right')
    ax.set_title('All39: source-label bootstrap top-lineage frequency\nGray = low signal / insufficient coverage')
    fig.colorbar(im,ax=ax,label='Descriptive stability, not a P value',shrink=.2);fig.tight_layout()
    fig.savefig(out/'all39_bootstrap_source_stability.png',dpi=160);plt.close(fig)
    vals={}
    ...
    report={'status':'DONE','scope':'ALL39 source-label descriptive resampling','cohorts':vals,
