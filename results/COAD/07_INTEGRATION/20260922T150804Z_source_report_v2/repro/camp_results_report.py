...
def build(c,d,z,blobs,manifest,out,config_path):
 ...
 def mapping(fig):
  ...
  now=set(z['relations'].gene);allg=set(z['genes'].gene);ax.bar(['直接关系','当前直接基因','条件或历史保留'],[len(z['relations']),len(now),len(allg-now)],color=['#087F8C','#337EAA','#9AA6B2']);ax.set_ylabel('数量（不同计数单位）');fig.tight_layout()
 mapping_counts=pd.DataFrame([dict(label=k,count=int(v),unit='入口代谢物') for k,v in z['mapping'].mapping_state.value_counts().items()]+[dict(label='直接关系',count=len(z['relations']),unit='关系'),dict(label='当前基因',count=z['relations'].gene.nunique(),unit='基因'),dict(label='条件或历史保留',count=len(set(z['genes'].gene)-set(z['relations'].gene)),unit='基因')])
 ...
 for i,module in enumerate(c.get('missing_modules',[])):
  frame=pd.DataFrame([module])
  def missing(fig,module=module):
   ax=fig.add_subplot(111);ax.axis('off');ax.text(.05,.7,module['title'],fontsize=20);ax.text(.05,.45,module['reason'],fontsize=13,wrap=True);ax.text(.05,.2,module['decision'],fontsize=12,wrap=True)
  save('missing_'+str(i),module['title'],frame,missing,module['narrative'],['config: missing module'],main=True)
 ...
 for key in ['all_relation_history','gene_membership','sc_cross_study','sc_feature_coverage','progress_summary']:
  if key in d:book[key[:31]]=d[key]
 payload={name:{'columns':list(df.columns),'rows':df.astype(object).where(pd.notna(df),'NA').values.tolist()} for name,df in book.items()}
 dump(out/'repro'/'workbook_data.json',payload)
 ...
 dep=['numpy','pandas','matplotlib','PyYAML','PyMuPDF']
